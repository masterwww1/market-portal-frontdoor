...
import { ROLE_META, getSubdomain } from '@/utils/subdomain';
      ...
      <AppBar position="static" color="primary" elevation={0}>
        <Toolbar sx={{ minHeight: 56, gap: 1 }}>
          <Typography
            ...
            sx={{ fontWeight: 800, textDecoration: 'none', color: 'inherit', letterSpacing: '-0.3px', mr: 1 }}
          >
            B2Bmarket
          </Typography>

          {/* Subdomain / portal pill */}
          {subdomain && roleMeta && (
            <span
          ...
          {isAuthenticated ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              <Typography variant="body2" sx={{ opacity: 0.85, fontSize: '13px', display: { xs: 'none', md: 'block' } }}>
                {user?.email}
              </Typography>
        ...
        >
              ...
              <>
                ...
                {portalNavItems.map(({ to, label, Icon }) => {
                  ...
                  return (
                    ...
                    >
                      <Icon sx={{ fontSize: 18, color: active ? '#4338ca' : 'inherit', opacity: active ? 1 : 0.6 }} />
                      {label}
                    </RouterLink>
                  );
                })}
          ...
          <div className="p-4 border-t border-gray-100">
            <p className="text-[10px] text-gray-300 font-medium text-center tracking-wide">
              {subdomain ? `${subdomain.toUpperCase()} PORTAL` : 'B2BMARKET'}
            </p>
          </div>
        </aside>
